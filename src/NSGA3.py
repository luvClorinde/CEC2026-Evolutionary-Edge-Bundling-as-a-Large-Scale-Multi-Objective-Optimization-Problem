...
class NSGA3Parameters:
    """Hyper-parameters controlling the NSGA-III run."""

    iterations: int = 750  # NSGA-III を実行する総反復回数を指定します。
    population_size: int | None = 500   # 各世代で扱う個体数を設定します（未指定なら参照方向数に揃える）。
    ...
    reference_direction_partitions: int = 12  # 参照方向の分割数（NSGA-III の基準方向生成に使用）。
...
### 問題の定義
class EdgeBundlingNSGAIII(FloatProblem):
    def __init__(
        self,
        bound: float,
    ...
    def name(self) -> str:
        return "EdgeBundlingNSGAIII"


### 実験定義
class NSGA3EdgeBundlingExperiment:
    """Bundle NSGA-III optimisation logic for a single graph."""
    def __init__(
        self,
        graph_name: str,
        objectives: Sequence[str],
        parameters: NSGA3Parameters,
        results_dir: Path,
        i: int = 0,
    ) -> None:
        ...
        self.problem = EdgeBundlingNSGAIII(
            VALUE_RANGE[i],
            self.module,
            self.functions,
            objectives,
            population_size=parameters.population_size or 1,
        )

        reference_directions = UniformReferenceDirectionFactory(
            n_dim=self.problem.number_of_objectives(),
            n_partitions=parameters.reference_direction_partitions,
        )
        ref_dirs = reference_directions.compute()
        population_size = (
            parameters.population_size
            if parameters.population_size is not None
            else len(ref_dirs)
        )
        self.max_evaluations = parameters.iterations * population_size
...
def main(i = "0", graphs = None, pc = None, iterations = None, population_size = None) -> None:
    ...
    for r, v in enumerate(VALUE_RANGE):
        for graph_name in graphs:
            ...
            experiment = NSGA3EdgeBundlingExperiment(
                graph_name,
                OBJECTIVES,
                parameters,
                result_dir,
