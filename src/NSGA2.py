...
### 問題の定義
class EdgeBundlingNSGAII(FloatProblem):
    def __init__(
        self,
        bound: float,
    ...
    def name(self) -> str:
        return "EdgeBundlingNSGAII"


### 実験定義
class NSGA2EdgeBundlingExperiment:
    """Bundle NSGA-II optimisation logic for a single graph."""
    def __init__(
        self,
        graph_name: str,
        objectives: Sequence[str],
        ...
        self.problem = EdgeBundlingNSGAII(
            VALUE_RANGE[i],
            self.module,
            self.functions,
            objectives,
...
def main(i = "0", graphs = None, pc = None, iterations = None, population_size = None) -> None:
    ...
    for r, v in enumerate(VALUE_RANGE):
        for graph_name in graphs:
            ...
            experiment = NSGA2EdgeBundlingExperiment(
                graph_name,
                OBJECTIVES,
                parameters,
                result_dir,
